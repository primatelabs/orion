...
#ifndef ORION_RENDERER_SCANLINE_H__
#define ORION_RENDERER_SCANLINE_H__
...
void render_scanlines_aa_solid(Rasterizer& ras,
  Scanline& sl,
  BaseRenderer& ren,
  const ColorT& color)
{
  if (ras.rewind_scanlines()) {
    ...
    while (ras.sweep_scanline(sl)) {
      ...
      for (;;) {
        int x = span->x;
        if (span->len > 0) {
          ren.blend_solid_hspan(x, y, (unsigned)span->len, ren_color, span->covers);
        } else {
          ren.blend_hline(x,
            y,
...
void render_scanline_aa(const Scanline& sl,
  BaseRenderer& ren,
  SpanAllocator& alloc,
...
template <class Rasterizer, class Scanline, class BaseRenderer, class ColorT>
void render_scanlines_bin_solid(Rasterizer& ras,
  Scanline& sl,
  BaseRenderer& ren,
  const ColorT& color)
{
...
template <class Scanline, class BaseRenderer, class SpanAllocator, class SpanGenerator>
void render_scanline_bin(const Scanline& sl,
  BaseRenderer& ren,
  SpanAllocator& alloc,
  SpanGenerator& span_gen)
{
