...
#ifndef ORION_PATH_STORAGE_H__
#define ORION_PATH_STORAGE_H__
...
void vertex_block_storage<T, S, P>::free_all()
{
  if (m_total_blocks) {
    ...
    while (m_total_blocks--) {
      pod_allocator<T>::deallocate(*coord_blk,
        block_size * 2 + block_size / (sizeof(T) / sizeof(unsigned char)));
      --coord_blk;
    }
...
template <class T, unsigned S, unsigned P>
vertex_block_storage<T, S, P>::vertex_block_storage(
  const vertex_block_storage<T, S, P>& v)
: m_total_vertices(0)
, m_total_blocks(0)
, m_max_blocks(0)
, m_coord_blocks(0)
...
template <class T, unsigned S, unsigned P>
const vertex_block_storage<T, S, P>& vertex_block_storage<T, S, P>::operator=(
  const vertex_block_storage<T, S, P>& v)
{
  remove_all();
  unsigned i;
...
void vertex_block_storage<T, S, P>::allocate_block(unsigned nb)
{
  ...
  m_coord_blocks[nb] = pod_allocator<T>::allocate(
    block_size * 2 + block_size / (sizeof(T) / sizeof(unsigned char)));
...
public:
  ...
  void curve4_rel(double dx_ctrl1,
    double dy_ctrl1,
    double dx_ctrl2,
    double dy_ctrl2,
    double dx_to,
  ...
  void join_path(VertexSource& vs, unsigned path_id = 0)
  {
    ...
    if (!is_stop(cmd)) {
      ...
      while (!is_stop(cmd = vs.vertex(&x, &y))) {
        m_vertices.add_vertex(x,
          y,
          is_move_to(cmd) ? unsigned(path_cmd_line_to) : cmd);
      }
    }
  }
...
template <class VC>
void path_base<VC>::arc_to(double rx,
  double ry,
  double angle,
  bool large_arc_flag,
  bool sweep_flag,
...
template <class VC>
void path_base<VC>::curve4(double x_ctrl1,
  double y_ctrl1,
  double x_ctrl2,
  double y_ctrl2,
  double x_to,
...
void path_base<VC>::invert_polygon(unsigned start)
{
  ...
  while (
    start < m_vertices.total_vertices() && !is_vertex(m_vertices.command(start)))
    ++start;
  ...
  while (
    end < m_vertices.total_vertices() && !is_next_poly(m_vertices.command(end)))
    ++end;
...
public:
  ...
  unsigned last_command() const
  {
    return m_vertices.size() ? m_vertices[m_vertices.size() - 1].cmd
                             : path_cmd_stop;
  }
...
// namespace orion
//{
//    typedef path_base<vertex_stl_storage<std::vector<vertex_d> > >
//    stl_path_storage;
//}
