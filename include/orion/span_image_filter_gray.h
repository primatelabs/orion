...
#ifndef ORION_SPAN_IMAGE_FILTER_GRAY_H__
#define ORION_SPAN_IMAGE_FILTER_GRAY_H__
...
public:
  ...
  void generate(color_type* span, int x, int y, unsigned len)
  {
    base_type::interpolator().begin(x + base_type::filter_dx_dbl(),
      y + base_type::filter_dy_dbl(),
      len);
    do {
      base_type::interpolator().coordinates(&x, &y);
      span->v =
        *(const value_type*)base_type::source().span(x >> image_subpixel_shift,
...
public:
  ...
  void generate(color_type* span, int x, int y, unsigned len)
  {
    ...
    int len_x_lr =
      (diameter * base_type::m_rx + image_subpixel_mask) >> image_subpixel_shift;
    ...
    do {
      ...
      for (;;) {
        ...
        x_hr = x_hr2;
        for (;;) {
          int weight = (weight_y * weight_array[x_hr] + image_filter_scale / 2) >>
                       downscale_shift;
