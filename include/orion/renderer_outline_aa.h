...
#ifndef ORION_RENDERER_OUTLINE_AA_H__
#define ORION_RENDERER_OUTLINE_AA_H__
...
public:
  ...
  , m_step(0)
  {
    orion::dda2_line_interpolator li(0,
      lp.vertical ? (lp.dy << orion::line_subpixel_shift)
                  : (lp.dx << orion::line_subpixel_shift),
      lp.len);
...
public:
  ...
  line_interpolator_aa3(renderer_type& ren,
...
public:
  ...
  void gamma(const GammaF& gamma_function)
  {
    ...
    for (i = 0; i < aa_scale; i++) {
      m_gamma[i] =
        value_type(uround(gamma_function(double(i) / aa_mask) * aa_mask));
    }
  }
...
public:
  ...
  // FIXME line_profile_aa& profile() { return *m_profile; }
